...
export default function Subheader({type,categorie,explore,display,shareData}:{type:string,categorie:string[],explore:any[],display:string,shareData:any}){
    let id = 1;
                    ...
                    <div className="flex justify-start items-start flex-col">
                        {
                            explore.map(e=>{
                                return (
                                <Link onClick={()=>{handleClick(type)}} to={type === 'serie'?'serie/explore/'+e.id:'film/explore/'+e.id} key={e.id} className="text-black pl-3 pt-1 pb-1 w-full hover:bg-gray-100 rounded-3xl">{e.name}</Link> 
                                )
                            })
                        }
                    </div>
                    ...
                    <div className="flex justify-start items-center gap-x-4 flex-wrap">
                        {
                            categorie.map(c=>{ return (
                                <Link onClick={()=>{handleClick(type)}} to={type === 'serie'?'serie/'+replaceWhiteSpaceOnLabel(c.toLowerCase())+'/'+id:'film/'+replaceWhiteSpaceOnLabel(c.toLowerCase())+'/'+id} key={c} className="text-black w-2/5 pl-3 pt-1 pb-1 hover:bg-gray-100 rounded-3xl">{c}</Link>
                            )})
                        }
                    </div>
                </div>
