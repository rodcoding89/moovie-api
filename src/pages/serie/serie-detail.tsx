  ...
  useEffect(()=>{
    const loadSerie = async()=>{
      ...
      for (let i = 0; i < saison; i++) {
        const item = <div style={{width:100/saison +'%'}} key={i} className=" cursor-pointer"><Link className="w-full min-w-[145px] block" to={'season/'+(i+1)} style={{maxWidth:180}}><img className="mb-2" src={image_base_url+filmDetail?.poster_path} alt="saison"/>Saison {i+1}</Link></div>
        ...
        <div className="bg-black">
            <section>
                <div>
                  ...
                  <div className="w-[90%] mx-auto flex justify-center items-center gap-x-10 relative z-10 mt-3 mb-16 flex-col">
                    ...
                    <div className="w-full flex-nowrap flex justify-center items-center gap-10 relative z-10">{!loading ? !error ? <div className="w-full relative">{
                    currentScroll > 0 && (
                      <span onClick={previous} className={`prev absolute z-10 top-[50%] left-[-30px] translate-y-[-50%] flex items-center justify-center text-[2.2em] cursor-pointer ${saison > 2 ? 'block' : 'hidden'}`}>
                        <i className="fa fa-angle-left" aria-hidden="true"></i>
                      </span>
                    )
                  }<div className="overflow-hidden w-full relative"><div className="flex justify-start items-start gap-5 transition-all duration-700 ease-linear" style={{width:180*saison +'px',transform:'translateX(-'+(currentScroll * 100 / saison)+'%)'}}>{listSaison}</div></div>{
                  currentScroll < saison && (
                    <span data-test-data={currentScroll} onClick={next} className={`next absolute z-10 right-[-30px] top-[50%] translate-y-[-50%] flex items-center justify-center text-[2.2em] cursor-pointer ${saison > 2 ? 'block' : 'hidden'}`}>
                      <i className="fa fa-angle-right" aria-hidden="true"></i>
                    </span>
                  )
                }</div> : <div className="w-full"><p className="text-center z-10 relative">Données indisponible pour le moment</p></div> : <div className="w-full flex items-center justify-center"><div className='loader after:!border-t-transparent after:!border-b-white after:!border-l-white after:!border-r-white'></div></div>}</div>
                  </div>
            ...
            <section>
                ...
                </div>
                {
                  !loading ? !error ? traillerLink ? <div className="z-10 relative w-full flex items-center justify-center flex-col"><div><h3 className="mb-5 mt-5 text-white text-[1.6em] bold max-730:text-center max-730:mx-5">Regarder un extrait de cette serie</h3><iframe className="w-[50vw] h-[350px]" src={traillerLink} title={traillerName} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen></iframe><h5 className=" text-yellow text-[1.2em] bold my-2">{data?.original_title ? data?.original_title : data?.name}</h5>
                    ...
                    <div className="poster cursor-pointer">
                      <img className="w-auto max-700:w-[75vw]" src={image_base_url+data?.backdrop_path} alt="poster" />
                ...
                <div className="my-5 mx-auto w-[90vw] z-10 relative mb-5">
                    ...
                    <div className="w-[90vw] mx-auto">{!loading ? !error ? <ProviderComponent listProvider={listProvider} providerStyle={providerStyle} left=" left-0 " right=" right-0 " movieType="serie"/> : <div className="w-full"><p className="text-center z-10 relative">Données indisponible pour le moment</p></div> : <div className="w-full flex items-center justify-center"><div className='loader after:!border-t-transparent after:!border-b-white after:!border-l-white after:!border-r-white'></div></div>}</div>
                </div>
                <div className="relative mx-auto z-10 w-[100%] mt-[50px] mb-10 flex gap-x-5 items-start max-730:flex-col">
                    ...
                    <div className="w-[70%] max-730:w-full">{!loading1 ? !error1 ? <MovieComponent listMovie={listMovie}/> : <div className="w-full"><p className="text-center z-10 relative">Données indisponible pour le moment</p></div> : <div className="w-full flex items-center justify-center"><div className='loader after:!border-t-transparent after:!border-b-white after:!border-l-white after:!border-r-white'></div></div>}</div>
                </div>
                <div className="relative my-5 z-10">
                    ...
                    <div className="ml-[5vw] max-730:ml-0">
                      {!loading1 ? !error1 ? <OtherMovie listMovie={listAuthorMovie}/> : <div className="w-full"><p className="text-center z-10 relative">Données indisponible pour le moment</p></div> : <div className="w-full flex items-center justify-center"><div className='loader after:!border-t-transparent after:!border-b-white after:!border-l-white after:!border-r-white'></div></div>}
                    </div>
                </div>  
            </section>
        </div>
