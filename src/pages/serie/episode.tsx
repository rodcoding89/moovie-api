        ...
        <div className="w-[100%] mx-auto flex max-885:flex-col">
            ...
            <div className="flex-1 bg-[#1a1a1a]">
                <div className="relative">
                    <PostSerie backImg={image_base_url+data?.filmDetail.poster_path} seriePostUrl={image_base_url+data?.filmDetail.poster_path} serieInfo={serieInfo}/>
                </div>
                <div className="mx-10">
                    ...
                    <div className="m-10">
                        <ul className="m-0 p-0">
                        {
                            provider.map((p:any,index:number)=>{
                                return (
                                    <li onClick={()=>{setCurrentProviderUrl(p.link?p.link:"");setCurrentProviderName(p.provider_name);setActiveProvider(index)}} className="flex justify-between items-center mb-3 cursor-pointer bg-[#2b2c2d] p-3 flex-wrap gap-3" key={`provider-${index+1}`}><span>Lien : {index+1}</span> <div className="flex justify-start items-center gap-x-2"><img className="w-[35px]" src={image_base_url+p.logo_path} alt="provider" /> <span>{p.provider_name}</span></div> <div className="flex flex-col items-center justify-center"><span>Qualité HD</span>{activeProvider === index && <em className="bg-green-600 px-[5px] py-[2px] text-[.65em] text-white rounded-xl not-italic">active provider</em>}</div> <span>Ajouté le 20/02/2025</span></li>
                                )
                            })
                        }
                        </ul>
