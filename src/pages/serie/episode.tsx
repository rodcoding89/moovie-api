...
export default function Episode(){
    ...
    useEffect(()=>{
        const loadEpisode = async()=>{
            ...
            setCurrentProviderUrl(`https://www.themoviedb.org/tv/${id}-${seriname?.toLowerCase().replaceAll(" ","-")}/watch`)
    ...
    return (
        <div className="w-[100%] mx-auto flex max-885:flex-col">
            <div className="w-[25%] bg-black max-885:w-full max-885:pb-10">
                <h4 className="text-white text-center py-5 bg-[#0c0c0c]">{data?.original_title ? data?.original_title : data?.name} {currentSeasonNumber}</h4>
                <div className="flex flex-col max-885:flex-row max-885:justify-center max-885:flex-wrap max-885:mx-5 max-h-[180vh] overflow-y-auto">
                    {
                ...
                </div>
                {
                    !error ? (
                        <div className="mx-10 relative" ref={providerRef}>
                            ...
                            {
                                currentProviderUrl ? <div className="bg-black w-full h-[350px] flex items-center justify-center relative"><Link className="bg-yellow text-black flex items-center justify-center py-3 px-4" to={currentProviderUrl} target="_blanc">Cliquer pour se rediriger vers le provider</Link><img className="w-[35px] h-auto absolute top-5 left-5" src={image_base_url+currentProviderLogo} alt="provider" /></div> : <div className="w-full h-[350px] flex items-center justify-center text-center text-white bg-black text-[2.5em]max-600:text-[1.7em] max-600:h-[250px]">Aucune video disponible pour ce provider</div>
                            }
